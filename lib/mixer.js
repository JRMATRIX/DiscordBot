...
class Mixer {
    ...
    maybeRefreshClient( tokens ) {
        ...
        console.log( now, tokens.data.expiresAt, expire );
        ...
        return new Promise( function( resolve, reject ) {
            ...
            if( now.getTime() >= expire.getTime() ) {
                Client.request( 'POST', 'oauth/token', {
                    ...
                    tokens.data.accessToken = response.access_token;
                    tokens.data.refreshToken = response.refresh_token;
                    tokens.data.expiresAt = now.setSeconds( now.getSeconds() + response.expires_in );
