...
/*========================================================================*/
    ...
    setMixerTokens( tokens ) {
//        console.log( tokens );
        return DB.set( 'mixer.tokens', tokens ).write();
    }
    
    getMixerTokens() {
        return DB.get( 'mixer.tokens' ).value();
    }
...
/*========================================================================*/
    ...
    setDefaultOptions() {
        ...
        defaultOptions.forEach( function( value, key ) {
            var set = DB.has( `config.${value[0]}` ).value();
            
            if( set === false || set === undefined ) {
                console.log( `config.${value[0]} is not set. Setting default value: ${value[1]}` );
                DB.set( `config.${value[0]}`, value[1] ).write(); 
            } 
        });
    }
    
    hasOption( option ) {
        var value = this.has( `config.${option}` ).value();
